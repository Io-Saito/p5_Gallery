        ...
        for (var j = _args.length - 1; j > 0; j -= 3) {
          if (_args[j - 1] != 0.0) {
            ...
            if (_args[j - 2] == 0.0) val = 1.0; // BGG: harmonic 0 (DC)
              ...
              val = Math.sin(TWO_PI * (_x[i] / (1.0 / _args[j - 2]) + _args[j] / 360.0));
            }
      ...
      for(var i in _x)
      {
        ...
        switch(_type) {
          ...
          case "even":
            ...
            break;
          case "low":
            _v[i] = Math.min(random(), random());
            break;
          case "high":
            _v[i] = Math.max(random(), random());
            break;
          case "triangle":
    ...
    p5.Gen.prototype.window = function(_x, _type, _args) {
      ...
      switch(_type) {
        ...
        case "hamming":
          var alpha = 0.54;
          var beta = 0.46;
          for(i in _x) _y[i] = alpha - beta * Math.cos(TWO_PI * _x[i]);
        break;
        ...
        case "hanningvonhannmeister": // very much no
          for(i in _x) _y[i] = 0.5 * (1-Math.cos(TWO_PI*_x[i]));
        break;
        ...
        case "triangle":
          for(i in _x) _y[i] = 1.0 - Math.abs((_x[i]-0.5)/0.5);
        break;
        ...
        case "bartlett-hann":
          ...
          var a1 = 0.48;
          var a2 = 0.38;
          for(i in _x) _y[i] = a0 - a1*Math.abs(_x[i] - 0.5) - a2*Math.cos(2*PI*_x[i]);
        break;
        case 4:
        ...
        case "blackman":
            ...
            // var a1 = 0.5;
            // var a2 = 0.08;
            for(i in _x) _y[i] = a0 - a1 * Math.cos(2.*PI*_x[i]) + a2 * Math.cos(4.*PI*_x[i]);
        break;
        ...
        case "generalizedblackman":
          ...
          {
            var pix = PI*_x[i];
            _y[i] = a0 - a1*Math.cos(2*pix) + a2*Math.cos(4*pix);
          }
        break;
        ...
        case "blackman-harris":
            ...
            var a2 = 0.14128;
            var a3 = 0.01168;
            for(i in _x) _y[i] = a0 - a1 * Math.cos(2.*PI*_x[i]) + a2 * Math.cos(4.*PI*_x[i]) + a3 * Math.cos(6.*PI*_x[i]);
        break;
        ...
        case "gaussian":
          ...
          var sigma = _args[0];
          for(i in _x) _y[i] = Math.exp(-0.5 * ((_x[i]-0.5) / (sigma*0.5)) * ((_x[i]-0.5) / (sigma*0.5)));
        break;
        ...
        case "kaiser":
          var alpha = 3.;
          for(i in _x) {
            var above = PI * alpha * Math.sqrt(1.0 - (2. * _x[i] - 1.0) * (2. * _x[i] - 1.0));
            var below = PI * alpha;
        ...
        // cosine window
        case "cosine":
          for(i in _x) _y[i] = Math.sin(PI*_x[i]);
        break;
        ...
        case "flattop":
          ...
          var a4 = 0.032;
          for(i in _x) {
            _y[i] = a0 - a1*Math.cos(2*PI*_x[i]) + a2*Math.cos(4*PI*_x[i]) - a3*Math.cos(6*PI*_x[i]) + a4*Math.cos(8*PI*_x[i]);
            _y[i] /= (a0 + a1 + a2 + a3 + a4);
          }
        break;
        ...
        case "tukey":
          ...
          for(i in _x)
          {
            _y[i] = 1.0;
            if (_x[i] <= ah) {
              _y[i] = 0.5 * (1.0 + Math.cos(PI*((2*_x[i]/_a)-1.0)));
            }
            else if (_x[i] > omah) {
              _y[i] = 0.5 * (1.0 + Math.cos(PI*((2*_x[i]/_a)-(2/_a)+1.0)));
            }
          }
        break;
        ...
        case "slidinggaussian":
          ...
          var sigma = _args[1] * 2.;
          for(var i in _x) _y[i] = Math.exp(0.0 - (sq(_x[i]*2.-1.0-dx) / (2.0*sigma*sigma)));
        break;
        ...
        case "adjustablecosine":
          ...
          for(i in _x)
          {
            _y[i] = 1.0;
            if (_x[i] <= _a) {
              _y[i] = 0.5 * (1.0 + Math.cos(PI*((_x[i]/_a)-1.0)));
            }
            else {
              _y[i] = 0.5 * (1.0 + Math.cos(PI*(((_x[i]-_a)/(1.0-_a)))));
            }
          }
        break;
        ...
        case "elliptic":
          ...
          for(i in _x)
          {
            _y[i] = 0;
            if (_x[i]<=_a){
              _y[i] = (1.0/_a) * Math.sqrt(sq(_a) - sq(_x[i]-_a));
            }
            else {
              _y[i] = (1.0/(1-_a)) * Math.sqrt(sq(1.0-_a) - sq(_x[i]-_a));
            }
          }
        break;
        ...
        case "hyperelliptic":
          ...
          for(i in _x)
          {
            ...
            if (_x[i]<=_a){
              _y[i] = (1.0/_a) * Math.pow( Math.pow(_a, pwn) - Math.pow(_x[i]-_a, pwn), 1.0/pwn);
            }
            else {
              _y[i] =  ((1.0/ (1-_a)))  * Math.pow( Math.pow(1.0-_a, pwn) - Math.pow(_x[i]-_a, pwn), 1.0/pwn);
            }
          }
        break;
        ...
        case "squircular":
          ...
          {
            _y[i] = 0;
            var pwn = Math.max(2, _n * 2.0);

            if (_x[i]<=_a){
              _y[i] = (1-_a) + Math.pow( Math.pow(_a, pwn) - Math.pow(_x[i]-_a, pwn), 1.0/pwn);
            }
            else {
              _y[i] = _a + Math.pow( Math.pow(1.0-_a, pwn) - Math.pow(_x[i]-_a, pwn), 1.0/pwn);
            }
          }
        break;
        ...
        case "poisson":
          if(!_args[0]) _args[0] = 0.5; // default center
          var tau = Math.max(_args[0], Number.EPSILON);
          for(var i in _x) _y[i] = Math.exp (0.0 - (Math.abs(_x[i] - 0.5))*(1.0/tau));
        break;
        case "hann-poisson":
        case "poisson-hann":
        case "hannpoisson":
        case "poissonhann":
          if(!_args[0]) _args[0] = 0.5; // default center
          var tau = 25.0 * Math.max(_args[0]*_args[0]*_args[0]*_args[0], Number.EPSILON); // nice control
          for(i in _x) {
            var hy = 0.5 * (1.0 - Math.cos(TWO_PI*_x[i]));
            var py = Math.exp (0.0 - (Math.abs(_x[i] - 0.5))*(1.0/tau));
            _y[i] = hy * py;
          }
        break;
        ...
        case "slidingpoissonhann":
          ...
          var tau = 25.0 * Math.max(_args[1]*_args[1]*_args[1]*_args[1], Number.EPSILON); // nice control
          for(i in _x) {
            var newx = constrain(_x[i] + (0.5 - _args[0]), 0, 1);
            var hy = 0.5 * (1.0 - Math.cos(TWO_PI*newx));
            var py = Math.exp (0.0 - (Math.abs(newx - 0.5))*(1.0/tau));
            _y[i] = hy * py;
          }
        break;
...
// Penner's Easing Functions:
  ...
  p5.Ease.prototype.elasticInOut = function(_x) {
  	if(_x < 0.5)
  	{
  		return(0.5 * Math.sin(13 * HALF_PI * (2 * _x)) * Math.pow(2, 10 * ((2 * _x) - 1)));
  	}
  	else
  	{
  		return(0.5 * (Math.sin(-13 * HALF_PI * ((2 * _x - 1) + 1)) * Math.pow(2, -10 * (2 * _x - 1)) + 2));
  	}
  }

  // overshooting cubic y = x^3-x*sin(x*pi)
  p5.Ease.prototype.backIn = function(_x) {
  	return(_x * _x * _x - _x * Math.sin(_x * PI));
  }
...
// Golan's Pattern Master Functions:
  ...
  p5.Ease.prototype.exponentialSmoothedStaircase = function(_x, _a, _n)
  {
    ...
    // See http://web.mit.edu/fnl/volume/204/winston.html
     ...
     var _y = 0;
     for (var i=0; i<_n; i++){
       _y += (1.0/(_n-1.0))/ (1.0 + Math.exp(fa*(((i+1.0)/_n) - _x)));
     }
     _y = constrain(_y, 0,1);
     return(_y);
  }
  ...
  p5.Ease.prototype.gompertz = function(_x, _a)
  {
    if(!_a) _a = 0.25; // default
    var min_param_a = 0.0 + Number.EPSILON;
    _a = Math.max(_a, min_param_a);

    var b = -8.0;
    var c = 0 - _a*16.0;
    var _y = Math.exp( b * Math.exp(c * _x));

    var maxVal = Math.exp(b * Math.exp(c));
    var minVal = Math.exp(b);
  ...
  p5.Ease.prototype.doubleOddPolynomialOgee = function(_x, _a, _b, _n)
  {
    ...
    var _y = 0;
    if (_x <= _a) {
      _y = _b - _b*Math.pow(1-_x/_a, p);
    }
    else {
      _y = _b + (1-_b)*Math.pow((_x-_a)/(1-_a), p);
    }
  ...
  p5.Ease.prototype.variableStaircase = function(_x, _a, _n)
  {
    ...
    var x0 = (Math.floor (_x*_n))/ _n;
    var x1 = (Math.ceil  (_x*_n))/ _n;
    var y0 = x0;
    var y1 = x1;
  ...
  p5.Ease.prototype.quadraticBezierStaircase = function(_x, _a, _n)
  {
    ...
    if ((_x <= p0x) && (_x >= x0)) {
      // left side
      if (Math.floor (_x*_n) <= 0){
        _y = map(_x, x0, px, y0, py);
      } else {
        if (Math.abs(_x - x0) < Number.EPSILON){
          // problem when x == x0 !
        }
        ...
        var interior = Math.max (0, za*za + om2a*zx);
        var t = (Math.sqrt(interior) - za)/om2a;
      ...
      // right side
      if (Math.ceil(_x*_n) >= _n) {
        _y = map(_x, px, x1, py, y1);
      }
      else {
        if (Math.abs(_x - x1) < Number.EPSILON){
          // problem when x == x1 !
        }
        ...
        var om2a = 1.0 - 2.0*za;
        if (Math.abs(om2a) < Number.EPSILON) {
      ...
      var om2a = 1.0 - 2.0*za;
      var t = (Math.sqrt(za*za + om2a*zx) - za)/om2a;
      var zy = (1.0-2.0*zb)*(t*t) + (2*zb)*t;
      zy *= (p1y - p0y);
      zy += p0y;
      _y = zy;
  ...
  p5.Ease.prototype.adjustableCenterDoubleExponentialSigmoid = function(_x, _a, _b)
  {
    ...
    var _y = 0;
    var w = Math.max(0, Math.min(1, _x-(_b-0.5)));
    if (w<=0.5){
  ...
  p5.Ease.prototype.doublePolynomialSigmoid = function(_x, _n)
  {
    ...
    if (_n%2 == 0){
      // even polynomial
      if (_x<=0.5){
        _y = Math.pow(2.0*_x, _n)/2.0;
      }
      else {
        _y = 1.0 - Math.pow(2*(_x-1.0), _n)/2.0;
      }
    }
      ...
      if (_x<=0.5){
        ...
        _y = 1.0 + Math.pow(2.0*(_x-1.0), _n)/2.0;
      }
  ...
  p5.Ease.prototype.doubleEllipticOgee = function(_x, _a, _b)
  {
    ...
    if (_x<=_a){
      _y = (_b/_a) * Math.sqrt(sq(_a) - sq(_x-_a));
    }
    else {
      _y = 1.0 - ((1.0-_b)/(1.0-_a))*Math.sqrt(sq(1.0-_a) - sq(_x-_a));
    }
  ...
  p5.Ease.prototype.doubleCubicOgee = function(_x, _a, _b)
  {
    ...
    var _y = 0;
    if (_x <= _a){
      _y = _b - _b*Math.pow(1.0-_x/_a, 3.0);
    }
    else {
      _y = _b + (1.0-_b)*Math.pow((_x-_a)/(1.0-_a), 3.0);
    }
  ...
  p5.Ease.prototype.doubleCircularSigmoid = function(_x, _a)
  {
    ...
    var _y = 0;
    if (_x<=_a) {
      _y = _a - Math.sqrt(_a*_a - _x*_x);
    }
    else {
      _y = _a + Math.sqrt(sq(1.0-_a) - sq(_x-1.0));
    }
  ...
  p5.Ease.prototype.doubleSquircularSigmoid = function(_x, _a, _n)
  {
    ...
    var _y = 0;
    if (_x<=_a) {
      _y = _a - Math.pow( Math.pow(_a,pwn) - Math.pow(_x,pwn), 1.0/pwn);
    }
    else {
      _y = _a + Math.pow(Math.pow(1.0-_a, pwn) - Math.pow(_x-1.0, pwn), 1.0/pwn);
    }
  ...
  p5.Ease.prototype.doubleQuadraticBezier = function(_x, _a, _b, _c, _d)
  {
    ...
    if (_x <= xmid){
      ...
      t = (Math.sqrt(aa*aa + om2a*xx) - aa)/om2a;
  ...
  p5.Ease.prototype.doubleEllipticSigmoid = function(_x, _a, _b)
  {
    ...
    if (_x<=_a){
      if (_a <= 0){
        ...
        _y = _b * (1.0 - (Math.sqrt(sq(_a) - sq(_x))/_a));
      }
    }
    else {
      if (_a >= 1){
        ...
        _y = _b + ((1.0-_b)/(1.0-_a))*Math.sqrt(sq(1.0-_a) - sq(_x-1.0));
      }
    }
  ...
  p5.Ease.prototype.normalizedErf = function(_x)
  {
    ...
    var _y = Math.sqrt (1.0 - Math.exp(0 - z2*(  (a*z2 + 4.0/PI) / (a*z2 + 1.0))));
    if (z < 0.0) _y = 0-_y;
  ...
  p5.Ease.prototype.normalizedInverseErf = function(_x)
  {
    ...
    var A = (2.0 / (PI *a)) + (Math.log(1.0-z2) / 2.0);
    var B = (Math.log(1.0-z2) / a);
    var _y = Math.sqrt( Math.sqrt(A*A - B) - A );
  ...
  p5.Ease.prototype.exponentialEmphasis = function(_x, _a)
  {
    ...
    if (_a < 0.5) {
      // emphasis
      _a = 2*(_a);
      var _y = Math.pow(_x, _a);
      return(_y);
    }
    else {
      // de-emphasis
      _a = 2*(_a-0.5);
      var _y = Math.pow(_x, 1.0/(1-_a));
      return(_y);
    }
  }
  ...
  p5.Ease.prototype.circularFillet = function(_x, _a, _b, _c)
  {
    ...
    if (_x <= arcStartX) {
      ...
      if (_x >= arcCenterX) {
        _y = arcCenterY - Math.sqrt(sq(arcRadius) - sq(_x-arcCenterX));
      }
      else {
        _y = arcCenterY + Math.sqrt(sq(arcRadius) - sq(_x-arcCenterX));
      }
  ...
  p5.Ease.prototype.circularArcThroughAPoint = function(_x, _a, _b)
  {
    ...
    function isPerpendicular(pt1x, pt1y, pt2x, pt2y, pt3x, pt3y)
    {
      ...
      if (Math.abs(xDelta_a) <= Number.EPSILON && Math.abs(yDelta_b) <= Number.EPSILON){
        return(false);
      }
      if (Math.abs(yDelta_a) <= Number.EPSILON){
        return(true);
      }
      else if (Math.abs(yDelta_b) <= Number.EPSILON){
        return(true);
      }
      else if (Math.abs(xDelta_a)<= Number.EPSILON){
        return(true);
      }
      else if (Math.abs(xDelta_b)<= Number.EPSILON){
        return(true);
      }
      else return(false);
    }
    ...
    function calcCircleFrom3Points(pt1x, pt1y, pt2x, pt2y, pt3x, pt3y, m)
    {
      ...
      if (Math.abs(xDelta_a) <= Number.EPSILON && Math.abs(yDelta_b) <= Number.EPSILON){
        ...
        m.dRadius = Math.sqrt(sq(m.centerX-pt1x) + sq(m.centerY-pt1y));
        return;
      }
      ...
      if (Math.abs(aSlope-bSlope) <= Number.EPSILON){	// checking whether the given points are colinear.
        return;
      }
    ...
    var _y = 0;
    if (_x >= m.centerX){
      _y = m.centerY - Math.sqrt(sq(m.dRadius) - sq(_x-m.centerX));
    }
    else{
      _y = m.centerY + Math.sqrt(sq(m.dRadius) - sq(_x-m.centerX));
    }
  ...
  p5.Ease.prototype.quadraticBezier = function(_x, _a, _b)
  {
    ...
    var om2a = 1.0 - 2.0*_a;
    var t = (Math.sqrt(_a*_a + om2a*_x) - _a)/om2a;
  ...
  p5.Ease.prototype.dampedSinusoid = function(_x, _a)
  {
    ...
    var t = _x;
    var _y = Math.pow(e, lambda*t) * Math.cos(omega*t + phi);
  ...
  p5.Ease.prototype.doubleCircularOgee = function(_x, _a)
  {
    ...
    var _y = 0;
    if (_x<=_a){
      _y = Math.sqrt(sq(_a) - sq(_x-_a));
    }
    else {
      _y = 1 - Math.sqrt(sq(1-_a) - sq(_x-_a));
    }
  ...
  p5.Ease.prototype.doubleSquircularOgee = function(_x, _a, _n)
  {
    ...
    var _y = 0;
    if (_x<=_a){
      _y = Math.pow( Math.pow(_a,pown) - Math.pow(_x-_a, pown), 1.0/pown);
    }
    else {
      _y = 1.0 - Math.pow( Math.pow(1-_a,pown) - Math.pow(_x-_a, pown), 1.0/pown);
    }
  ...
  p5.Ease.prototype.normalizedLogitSigmoid = function(_x, _a)
  {
    ...
    var _y    = 1.0 / (1.0 + Math.exp(0 - (_x-0.5)*_a    ));
    var miny = 1.0 / (1.0 + Math.exp(  0.5*_a    ));
    var maxy = 1.0 / (1.0 + Math.exp( -0.5*_a    ));
  ...
  p5.Ease.prototype.normalizedLogit = function(_x, _a)
  {
    ...
    var minx = 1.0 / (1.0 + Math.exp(  0.5*_a    ));
    var maxx = 1.0 / (1.0 + Math.exp( -0.5*_a    ));
    _x = map(_x, 0,1, minx, maxx);

    var _y = Math.log (_x / (1.0 - _x)) ;
    _y *= 1.0/_a;
    _y += 0.5;
  ...
  // =============================================================================
    ...
    // =============================================================================
      ...
      {
        // intermediates
        this.A = Math.sqrt(Math.pow(10, this.dB/20)); // amplitude
        this.w0 = 2*PI*this.f0/this.fs; // filter increment in radians
        this.cw0 = Math.cos(this.w0); // cosine of w0 - precompute
        this.sw0 = Math.sin(this.w0); // sine of w0 - precompute
        this.alpha = Math.sin(this.w0)/(2*this.Q); // alpha term - precompute
        this.soff = 2*Math.sqrt(this.A)*this.alpha; // shelving offset - precompute
   ...
   */
   p5.FastFourierTransform = function(_bufsize, _fs, _hopsize) {
     ...
     for (i = 0; i < this.bufferSize; i++) {
       ...
       this.sinTable[i] = Math.sin(-PI/i);
       this.cosTable[i] = Math.cos(-PI/i);
     }
   ...
   p5.FastFourierTransform.prototype.calculateFrequency = function() {
    ...
    p5.FastFourierTransform.prototype.forward = function(_buffer) {
     ...
     var k = Math.floor(Math.log(this.bufferSize) / Math.LN2);

     if (Math.pow(2, k) !== this.bufferSize) { throw "buffer size must be a power of 2."; }
  ...
  p5.prototype.imap = function(_x, _a, _b, _c, _d) {
    return(constrain(Math.floor(map(_x, _a, _b, _c, _d)), Math.min(_c,_d), Math.max(_c, _d)-1));
  }

  // number wrapping (courtesy of @pd-l2ork puredata [pong])
  p5.prototype.wrap = function(_x, _min, _max) {
    _a = Math.min(_min, _max);
    _b = Math.max(_min, _max);
    var _y;
    var _r = _b-_a; // range
  ...
  p5.prototype.fold = function(_x, _min, _max) {
    ...
    if(_x < _b && _x >= _a) { // normal
      ...
      if(_x < _a) {
        ...
        var _m = Math.floor(_d/_r); // case where input is more than a range away from minval
        ...
        var _m  = Math.floor(_d/_r); // case where input is more than a range away from maxval
  ...
  p5.prototype.normalizeArray = function(_array) {
    var _max = Math.max(Math.max(_array), Math.abs(Math.min(_array)));
  ...
  p5.prototype.resizeArray = function(_array, _newlen) {
    ...
    for(var i = 0;i<_newlen;i++)
    {
      ...
      var a = Math.floor(aptr);
      var b = Math.ceil(aptr);
      var l = aptr%1.0;
